...
Set up the broker intro for Thursday or Friday this week
THEIRS
Northwind: send site proposal data by Sep 9`;
...
test("granolaAnswer drops chain of thought and keeps the ledger", () => {
  ...
  assert.match(answer, /broker intro/);
});
...
test("isPromptEcho catches the four-bullet instruction fragment", () => {
  ...
  assert.equal(isPromptEcho("Set up the broker intro for Thursday"), false);
});
